...
configs = {
    ...
    'dinov3_vitb16': {
        ...
        "patch_size": 16,
        "in_chans": 3,
        "pos_embed_rope_base": 100,           # DINOv3 標配：Rotary Positional Embedding
        "pos_embed_rope_normalize_coords": "separate",
        "pos_embed_rope_rescale_coords": 2,
        "pos_embed_rope_dtype": "fp32",
        "embed_dim": 768,                    # Base 模型維度
        "depth": 12,                         # Base 模型層數
        "num_heads": 12,                     # 每個 head 維度 64 (768/12)
        "ffn_ratio": 4,                  # 重要：SwiGLU 下的 FFN 擴展比例通常為 8/3 ≈ 2.66
        "qkv_bias": True,
        "drop_path_rate": 0.0,
        "layerscale_init": 1.0e-05,
        "norm_layer": "layernormbf16",
        "ffn_layer": "mlp",               # 必須使用 swiglu
        "ffn_bias": True,
        "proj_bias": True,
        "n_storage_tokens": 4,               # Register tokens，消除特徵偽影
        "mask_k_bias": True,
        "pretrained": True,
        "weights": Weights.LVD1689M,
        "compact_arch_name": "vitbplus",
        "check_hash": False,
    }
}
