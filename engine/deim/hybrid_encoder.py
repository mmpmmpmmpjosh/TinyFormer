...
__all__ = ['HybridEncoder','HybridEncoder_4Scale']
...
class RepNCSPMSP4(nn.Module):
    ...
    # 
    def forward_chunk(self, x):
        y = list(self.cv1(x).chunk(2, 1))        # [y_chain, y_short]
    ...
    # 
    def forward(self, x):
        y = list(self.cv1(x).split((self.c, self.c), 1))  # view
        out_cv2 = self.cv2(y[0])
        out_cv3 = self.cv3(out_cv2)
        fused = torch.cat([out_cv2, out_cv3, y[1]], 1)
        return self.cv4(fused)
...
class RepNCSPMSP5(nn.Module):
    def __init__(self, c1, c2, c3, c4, n=3, bias=False, act="silu"):
        ...
        self.c = c3 // 2
        self.cv1 = ConvNormLayer_fuse(c1, c3, 1, 1, bias=bias, act=act)
        self.cv2 = nn.Sequential(CSPLayer2(self.c, c4, n, 1, bias=bias, act=act, bottletype=VGGBlock))
        self.cv3 = nn.Sequential(CSPLayer2(c4, c4, n, 1, bias=bias, act=act, bottletype=VGGBlock))
        self.cv4 = ConvNormLayer_fuse(self.c + 2*c4, c2, 1, 1, bias=bias, act=act)
...
class HybridEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        SCDown_Conv = nn.Sequential(SCDown(hidden_dim, hidden_dim, 3, 2))#ReOrg()

        c1, c2, c3, c4, num_blocks = input_dim, hidden_dim, hidden_dim*2, round(expansion * hidden_dim // 2), round(3 * depth_mult)
        if version == 'dfine':
            Fuse_Block = RepNCSPMSP4(c1=c1, c2=c2, c3=c3, c4=c4, n=num_blocks, act=act, csp_type=csp_type)
        elif version == 'deim':
            Fuse_Block = RepNCSPMSP5(c1=c1, c2=c2, c3=c3, c4=c4, n=num_blocks, act=act)
        else:       # RT-DETR
        ...
        #Fuse_Block_Down = RepNFastMSP5(c1=c2*5, c2=c2, c3=c3, c4=c4, n=num_blocks, act=enc_act,n_div=n_div,mlp_ratio= mlp_ratio)
        # top-down fpn
...
class HybridEncoder_4Scale(nn.Module):
    ...
    def __init__(self,
        ...
        self.out_channels = [hidden_dim for _ in range(len(out_indices))]
        self.out_strides = [feat_strides[i] for i in out_indices]
        ...
        c2_fpn = hidden_dim
        c3_fpn = hidden_dim * 2 
        c4_fpn = round(expansion * hidden_dim // 2)
        FPN_Block = RepNCSPMSP5(c1=c1_fpn, c2=c2_fpn, c3=c3_fpn, c4=c4_fpn, n=num_blocks, act=act)
        ...
        c2_pan = hidden_dim
        c3_pan = hidden_dim * 2
        c4_pan = round(expansion * hidden_dim // 2)
        PAN_Block = RepNCSPMSP5(c1=c1_pan, c2=c2_pan, c3=c3_pan, c4=c4_pan, n=num_blocks, act=act)
    ...
    def forward(self, feats):
        ...
        loop_end_idx = 1
        for idx in range(len(self.in_channels) - 2, loop_end_idx - 1, -1):
            feat_current = proj_feats[idx]      
            feat_low = proj_feats[idx - 1]      
            
            block_idx = (len(self.in_channels) - 2) - idx

            # Lateral Conv & Upsample
            feat_high_lateral = self.lateral_convs[block_idx](current_high_feat)
            feat_high_up = F.interpolate(feat_high_lateral, scale_factor=2., mode='nearest')
            ...
            processed_feat = self.fpn_blocks[block_idx](fused_feat)
            out = processed_feat 
